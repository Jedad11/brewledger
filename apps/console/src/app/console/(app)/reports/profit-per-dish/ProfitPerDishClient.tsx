...
export function ProfitPerDishView({ report }: { report: ProfitPerDishReport }) {
  ...
  // a prop changes") rather than in a useEffect, which would setState
  // synchronously after the first render and trigger a second, avoidable
  // render pass for every period change.
  const [prevReport, setPrevReport] = React.useState(report);
