 ...
 * column (packages/db/migrations/0041) the caller's concern, not this
 * module's.
 */
export function toBaseUnit(
  ...
  if (fromUnit === "pack") {
    if (packSize === undefined || packSize === null || !(packSize > 0)) {
      throw new Error(
        `toBaseUnit: "pack" requires a positive packSize (pieces per pack) -- none given. This is the ingredient's own pack_size column (packages/db/migrations/0041_ingredient_master_conversions.sql), not a default this function may invent.`,
      );
    }
    return qty * packSize;
  }
