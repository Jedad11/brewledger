...
//
// Drives packages/db/migrations/0036_ingredient_master_conversions.sql (the
  ...
  if (!dbAvailable) {
    ...
    console.warn(
      "\n[ingredient_master.test.ts] SKIPPING — no local Postgres reachable at " +
        ...
        "stack predates migration 0036) first. This is a SKIP, not a pass.\n",
    );
    return;
  }
...
describe("WBS 6.5 — ingredient deletion is blocked at the DB layer when referenced", () => {
  ...
  it("blocked by a stock_ledger reference (tightened CASCADE -> RESTRICT, 0036)", async () => {
    if (!dbAvailable) return;
    await withRollback(async (client, tx) => {
